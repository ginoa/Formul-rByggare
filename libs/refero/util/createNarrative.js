...
const createNarrativeForItem = (qi) => {
    let narrative = '';
...
const getAnswerAsString = (answer) => {
    if (!answer) {
        return '';
    }
    ...
    if (codingValue !== null && codingValue !== undefined && codingValue !== '' && codingValue !== constants_1.OPEN_CHOICE_ID) {
        return codingValue;
    }
    if (answer.valueDate) {
        return (0, date_core_1.parseDate)(answer.valueDate).toISOString();
    }
    if (answer.valueDateTime) {
        return (0, date_core_1.parseDate)(answer.valueDateTime).toISOString();
    }
    return '';
};
//# sourceMappingURL=createNarrative.js.map